...
Item.prototype = {
    ...
    setState: function(direction){
        this.dom.css({transform: 'translateY('+this[direction+'State']+'px)'})
    },
    slideIn: function(direction, callback){
        TweenLite.to(this.dom[0], this['durationIn'+direction]/1000, {y:0})
    },
    slideOut: function(direction, callback){
        TweenLite.to(this.dom[0], this['durationOut'+direction]/1000, {y:this[direction+'State']})
    }
}
...
var pageAnimateArray = [{
    ...
    durationOutDown: 500,
    init: function(){
        this.dom.css({transform: 'translateY('+this.DownState+'px)'})
    ...
    slideIn: function(direction, cur, pages){
        this.show();
        TweenLite.to(this.dom[0], this['durationIn'+direction]/1000, {
            y: 0,
            onComplete: function(){
                switch(direction){
                    case 'Up':
    ...
    slideOut: function(direction, cur, pages){
        var self = this;
        TweenLite.to(this.dom[0], this['durationOut'+direction]/1000, {
            y: this[direction+'State'],
            onComplete: function(){
                self.hide();
            }
        })
